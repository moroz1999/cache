...
/**
 * The OpcacheFile cache stores data using regular files.
 */

namespace fluxbb\cache\modules;

class OpcacheFile extends \fluxbb\cache\Cache
{
     ...
     *
     * @param $config
     * @throws \fluxbb\cache\Exception
     * @internal param \fluxbb\cache\modules\The $dir directory in which to store cache files. Must be
	...
	protected function _set($key, $data, $ttl)
	{
		if (@file_put_contents($this->dir.$this->key($key).$this->suffix, $data) === false)
		{
            ...
            opcache_invalidate($this->dir.$this->key($key).$this->suffix);
        }
	}
	...
	protected function loadFlat($key)
	{
        if (is_file($this->dir.$this->key($key).$this->suffix))
        {
            return include($this->dir.$this->key($key).$this->suffix);
        }
        return false;
	}
